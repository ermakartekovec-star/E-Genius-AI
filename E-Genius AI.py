...
WEB_APP_URL = "https://ermakartekovec-star.github.io/Web-App/"
...
def setup_bot_handlers():
    ...
    def web_app_handler(message):
        """Открытие Web App"""
        user_id = message.from_user.id
        log_event("WEB_APP", user_id)
        
        markup = types.InlineKeyboardMarkup()
        web_app_button = types.InlineKeyboardButton("🌐 Открыть Web App", url=WEB_APP_URL)
        markup.add(web_app_button)
        
        bot.send_message(message.chat.id,
                        f"<b>👌 Веб-приложение E-Genius AI</b>\n\n"
    ...
    def show_my_emails_handler(message):
        ...
        # ОРИГИНАЛЬНАЯ ЛОГИКА: Разрешаем всем авторизованным пользователям
        access_level = check_user_access(user_id)
...
"""
    ...
    def handle_private_content_for_chats(message):
        ...
        menu_buttons = [
            "🔐 Войти в систему", "📧 Добавить email", "👌Web App", "📤 Отправить сообщение",
...
def process_email_broadcast(message):
    ...
    for email in emails:
        if send_email(email, "Новости от E-Genius AI System", broadcast_message):
            success_count += 1
        else:
            fail_count += 1
        time.sleep(1)
...
def show_guest_menu(message):
    ...
    btn_webapp = types.KeyboardButton("👌Web App")
    markup.add(btn_auth, btn_email, btn_ai, btn_stop_ai, btn_webapp)
...
✅ 👌Web App - веб-приложение с дополнительными функциями
...
def show_admin_menu(message):
    ...
    markup.add(btn_pc, btn_send, btn_chats, btn_email, btn_emails, 
               btn_platon, btn_broadcast, btn_ai, btn_stop_ai, btn_settings, btn_webapp)
...
def show_platon_menu(message):
    ...
    btn_webapp = types.KeyboardButton("👌Web App")
    markup.add(btn_ai, btn_stop_ai, btn_email, btn_webapp)
...
if __name__ == "__main__":
    print(f"\n{'=' * 60}")
    print(f"🚀 Запуск E-Genius AI System")
    print(f"{'=' * 60}")
    ...
    print("   5. 👌Web App - веб-приложение с дополнительными функциями")
